...
SELECT 
    ...
    ROUND(SUM(CASE WHEN t.is_approved THEN t.amount ELSE 0 END), 2) as approved_value_usd,
    ROUND(SUM(CASE WHEN NOT t.is_approved THEN t.amount ELSE 0 END), 2) as declined_value_usd
...
WITH current_metrics AS (
    SELECT 
        ROUND(AVG(CASE WHEN t.is_approved THEN 1 ELSE 0 END) * 100, 2) as current_approval_rate,
        ROUND(AVG(t.composite_risk_score), 3) as current_risk_score,
        ROUND(AVG(t.solution_cost), 2) as current_avg_cost
    FROM payments_lakehouse.silver.payments_enriched_stream t
...
SELECT 
    t.recommended_solution_name,
    COUNT(*) as txn_count,
    ROUND(AVG(CASE WHEN t.is_approved THEN 1 ELSE 0 END) * 100, 2) as approval_rate_pct,
    ROUND(AVG(t.approval_uplift_pct), 2) as avg_uplift_pct,
    ...
    ROUND((AVG(t.approval_uplift_pct) * AVG(t.amount) / 100 - AVG(t.solution_cost)) / NULLIF(AVG(t.solution_cost), 0), 2) as roi_ratio
FROM payments_lakehouse.silver.payments_enriched_stream t
WHERE t.timestamp >= CURRENT_TIMESTAMP() - INTERVAL 7 DAYS
GROUP BY t.recommended_solution_name
HAVING COUNT(*) > 100
...
SELECT 
    ...
    ROUND(AVG(txn.amount), 2) as avg_amount_usd,
    ROUND(SUM(txn.amount), 2) as total_declined_value_usd,
